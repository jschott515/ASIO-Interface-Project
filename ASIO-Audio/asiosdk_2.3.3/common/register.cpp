...
LONG RegisterAsioDriver (CLSID clsid,char *szdllname,char *szregname,char *szasiodesc,char *szthreadmodel)
{
	...
	if (newregentry) {
		...
		sprintf(szregpath,"%s\\%s",SZREGSTR_CLSID,szclsid);
		createRegPath (HKEY_CLASSES_ROOT,szregpath,(char*)SZREGSTR_INPROCSERVER32);
		...
		createRegStringValue(HKEY_CLASSES_ROOT,szregpath, (char*)SZREGSTR_THREADINGMODEL,szthreadmodel);
	}
	...
	if (rc) {
		...
		createRegPath (HKEY_LOCAL_MACHINE,szregpath, (char*)SZREGSTR_ASIO);
	}
	...
	sprintf(szregpath,"%s\\%s\\%s",SZREGSTR_SOFTWARE,SZREGSTR_ASIO,szregname);
	createRegStringValue(HKEY_LOCAL_MACHINE,szregpath, (char*)SZREGSTR_CLSID,szclsid);
	createRegStringValue(HKEY_LOCAL_MACHINE,szregpath, (char*)SZREGSTR_DESCRIPTION,szasiodesc);
